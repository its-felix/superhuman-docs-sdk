# superhuman-docs-async Rust SDK
...
```rust,no_run
use superhuman_docs_async::{
    operations, Client, ClientOptions, Error, Request, Response, Transport,
    TransportFuture,
};
...
impl Transport for MyTransport {
    fn send_request(&self, request: Request) -> TransportFuture<'_> {
        Box::pin(async move {
            // Adapt `request.method`, `request.url`, and the optional JSON
            // `request.body` to your async HTTP stack. Add authentication here,
            // then return the status and response bytes.
            todo!()
        })
    }
}

let client = Client::new(ClientOptions::new(MyTransport))?;
...
Every generated operation is async, builds a `Request`, and dispatches it through
`Client::send_request`. Implement `Transport` to integrate an async HTTP client.
`ClientOptions::with_base_url` can be
