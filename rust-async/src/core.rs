...
pub type TransportFuture<'a> = Pin<Box<dyn Future<Output = Result<Response, Error>> + Send + 'a>>;
...
impl Transport for MissingTransport {
    fn send_request(&self, _request: Request) -> TransportFuture<'_> {
        Box::pin(async { Err(Error::MissingTransport) })
    }
}
...
impl Client {
    ...
    pub(crate) async fn execute<T: DeserializeOwned>(&self, request: Request) -> Result<T, Error> {
        let (operation, body) = self.receive(request).await?;
    ...
    async fn receive(&self, request: Request) -> Result<(&'static str, Vec<u8>), Error> {
